...
public class MultiplexSet implements Comparable<MultiplexSet>  {
	...
	public boolean addPair(String targetName, PrimerPair pair) throws Exception {
		...
		// specific // mispriming to other targets that we have in this set
//		if(!parentResult.calcMispriming(targetName, pair,set.keySet()))
//		{
//			return false;
//		}
		// also test what we have agianst the new one
		List<String> newTargetList = new ArrayList<String>(); 
		newTargetList.add(targetName);
		for(Entry<String, PrimerPair> oldPair : set.entrySet())
		{
//			if(!parentResult.calcMispriming(oldPair.getKey(), oldPair.getValue(),newTargetList))
//			{
//				return false;
//			}
		}
					...
					if(checkSetProductsLen(newProductLen))
					{
						...
						if(newLeft != null && newLeft.OK_OR_MUST_USE())
						{
							int pairStatus = newPair.characterize_pair(parentResult.sourcePairsResult.get(targetName),  newLeft, pair.right,  
									parentResult.p3MultiplexSearcher.dpal_arg_to_use, parentResult.p3MultiplexSearcher.thal_arg_to_use, false);
...
//	double compl_end = 0;
	private double checkDimers(PrimerRecord p1, PrimerRecord p2) throws ThermodynamicAlignmentException {
		...
		s2_rev = p2.getOligoRevSeq();
		 ...
		 double compl_any = LibPrimer3.align_thermod(s1, s2_rev, this.parentResult.p3MultiplexSearcher.thal_arg_to_use.any);
...
//	         }
	      ...
	      double compl_endTemp = LibPrimer3.align_thermod(s1, s2_rev, this.parentResult.p3MultiplexSearcher.thal_arg_to_use.end1);
	      if (compl_endTemp > this.parentResult.pa.getPairComplEndTH())
	    	   return Double.POSITIVE_INFINITY;
	      compl_end = Double.max(compl_end, compl_endTemp);
	      
	      compl_endTemp = LibPrimer3.align_thermod(s1, s2_rev, this.parentResult.p3MultiplexSearcher.thal_arg_to_use.end2); /* Triinu Please check */
	      if (compl_endTemp > this.parentResult.pa.getPairComplEndTH())
	    	  return Double.POSITIVE_INFINITY;
	      compl_end = Double.max(compl_end, compl_endTemp);	     
	      
	      
	      compl_endTemp = LibPrimer3.align_thermod(s1, s2_rev, this.parentResult.p3MultiplexSearcher.thal_arg_to_use.end1);
	      if (compl_endTemp > this.parentResult.pa.getPairComplEndTH())
	    	  return Double.POSITIVE_INFINITY;
	      compl_end = Double.max(compl_end, compl_endTemp);
