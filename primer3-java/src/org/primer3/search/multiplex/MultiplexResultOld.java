...
public class MultiplexResultOld {
	...
	ArrayList<TargetCoverSet> coversSet = new ArrayList<MultiplexResultOld.TargetCoverSet>();
	...
	public MultiplexResultOld(P3MultiplexSearch p3MultiplexSearch,String groupName,  P3GlobalSettings pa) {
...
//			return true;
		...
		for( Entry<String,  P3RetVal> kValue : sourcePairsResult.entrySet() )
		{
			...
			for(PrimerRecord h : retval.fwd.oligo) {
				 h.calcSpecific(targets,kValue.getKey(),  REVERSE_PREFIX_TARGETS , pa.isLibAmbiguityCodesConsensus() ?  p3MultiplexSearcher.dpal_arg_to_use.local_end_ambig
							: p3MultiplexSearcher.dpal_arg_to_use.local_end);
				 if(h.targetSpecificIndex.size() > 0 && h.getIsTargetSpecific())
				 {
					 leftToSpRight.fwd.add_oligo_to_oligo_array(h); 
				 }
				 if(h.getIsTargetSpecific() && h.targetSpecificIndex.size() == 0) {
	...
	public void addSet(MultiplexSet res) {
		multiplexSets.add(res);		
	}
